...
class Modbus(object):
    def __init__(self, itf, addr_list: list):
        ...
        self._available_register_types = ["COILS", "HREGS", "IREGS", "ISTS", "SREGS"]
        self._register_dict = dict()
        for reg_type in self._available_register_types:
            self._register_dict[reg_type] = dict()
        self._default_vals = dict(
            zip(self._available_register_types, [True, 999, 999, True, ""])
        )
    ...
    def add_coil(self, address: int, value: Union[bool, List[bool]] = False) -> None:
        """
    ...
    def add_ist(self, address: int, value: Union[bool, List[bool]] = False) -> None:
        """
    ...
    def add_sreg(self, address: int, value: Union[int, List[int]] = 0) -> None:
        """
        Add an input register to the modbus register dictionary.
        ...
        :type       value:    Union[int, List[int]], optional
        """
        self._set_reg_in_dict(reg_type="SREGS", address=address, value=value)

    def remove_sreg(self, address: int) -> Union[None, int, List[int]]:
        """
        Remove an input register from the modbus register dictionary.
        ...
        :rtype:     Union[None, int, List[int]]
        """
        return self._remove_reg_from_dict(reg_type="SREGS", address=address)

    def set_sreg(self, address: int, value: Union[int, List[int]] = 0) -> None:
        """
        Set the input register value.
    ...
    def get_sreg(self, address: int) -> Union[int, List[int]]:
        """
        Get the input register value.
    ...
    def sregs(self) -> dict_keys:
        ...
        :rtype:     dict_keys
        """
        return self._get_regs_of_dict(reg_type="SREGS")
    ...
    def _set_reg_in_dict(
        ...
        if not self._check_valid_register(reg_type=reg_type):
            raise KeyError(
                "{} is not a valid register type of {}".format(
    ...
    def _remove_reg_from_dict(
        self, reg_type: str, address: int
    ...
    def _get_reg_in_dict(
        ...
        if address in self._register_dict[reg_type]:
            ...
            raise KeyError(
                "No {} available for the register address {}".format(reg_type, address)
            )
    ...
    def _set_changed_register(
        ...
        if reg_type in self._changeable_register_types:
            content = {"val": value, "time": time.monotonic_ns() // 1000}
            self._changed_registers[reg_type][address] = content
        else:
            raise KeyError("{} can not be changed externally".format(reg_type))

    def _remove_changed_register(
        self, reg_type: str, address: int, timestamp: int
    ) -> bool:
        """
        ...
        if reg_type in self._changeable_register_types:
            _changed_register_timestamp = self._changed_registers[reg_type][address][
                "time"
            ]
            ...
            raise KeyError(
                "{} is not a valid register type of {}".format(
                    reg_type, self._changeable_register_types
                )
            )
    ...
    def process(self) -> bool:
        ...
        if request.function == ModbusConst.READ_COILS:
            ...
            reg_type = "ISTS"
            req_type = "READ"
        elif request.function == ModbusConst.REPORT_SERVER_ID:
            # function 17 - return ID address
            reg_type = "ID"
            req_type = "READ"
    ...
    def _create_response(self, request: Request, reg_type: str):
        ...
        """
        val = self._register_dict[reg_type][request.register_addr]
        if type(val) is list:
            return val
        else:
            return [val]
    ...
    def _process_read_access(self, request: Request, reg_type: str) -> None:
        ...
        :type       reg_type:  str
        """
        if request.function in [ModbusConst.REPORT_SERVER_ID]:
            request.send_response(self._addr_list)
        elif request.register_addr in self._register_dict[reg_type]:
    ...
    def _process_write_access(self, request: Request, reg_type: str) -> None:
        ...
        if address in self._register_dict[reg_type]:
            ...
            if valid_register:
                self._set_changed_register(
                    reg_type=reg_type, address=address, value=val
                )
        else:
            request.send_exception(ModbusConst.ILLEGAL_DATA_ADDRESS)

    def setup_registers(
        self, registers: dict = dict(), use_default_vals: bool = True
    ) -> None:
        if len(registers):
            for reg_type, default_val in self._default_vals.items():
                if reg_type in registers:
                    for reg, val in registers[reg_type].items():
                        ...
                        if reg_type == "COILS":
                            self.add_coil(address=address, value=value)
                        elif reg_type == "HREGS":
                            self.add_hreg(address=address, value=value)
                        elif reg_type == "ISTS":
                            self.add_ist(address=address, value=value)
                        elif reg_type == "IREGS":
                            self.add_ireg(address=address, value=value)
                        elif reg_type == "SREGS":
                            self.add_sreg(address=address, value=value)
                        else:
