...
class Client:
    ...
    def __init__(self, modbus, repl=None, delay=0.1):
        self.modbus = modbus
        self.ists = {}
        self.coils = {}
        self.iregs = {}
    ...
    def parse(self, object):
        strs = str(object).strip("<>").split(" ")
        retval = strs[0].lower()
        if retval in ["module", "object"]:
            try:
                retval = strs[1].lower().strip("'\"")
            except:
                pass
        return retval

    def str_reg(self, reg, regval):
        try:
            object, attr, format = regval[0]
        except:
            object, attr, format = regval
        return f"{self.parse(object)},{attr},{reg},{format}"

    def dump_regs(self):
        types = list(self.MOD_FUNCS.keys())

        for type in types:
            for reg, val in getattr(self, type.lower()).items():
                print(self.str_reg(reg, val))
