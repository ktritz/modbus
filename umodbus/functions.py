...
def read_id():
    return struct.pack(">BH", Const.REPORT_SERVER_ID, 0)
...
def write_single_register(register_address, register_value, signed=True):
    ...
    return struct.pack(
        ">BH" + fmt, Const.WRITE_SINGLE_REGISTER, register_address, register_value
    )
...
def validate_resp_data(
    data, function_code, address, value=None, quantity=None, signed=True
):
...
def response(
    ...
    if function_code in [
        ...
        fmt = "B" * len(output_value)
        return struct.pack(
            ">BB" + fmt, function_code, ((len(value_list) - 1) // 8) + 1, *output_value
        )
        ...
        return struct.pack(
            ">BHH", function_code, request_register_addr, request_register_qty
        )
